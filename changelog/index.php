<?php include $_SERVER['DOCUMENT_ROOT'] . '/_top.php'; ?>
<!DOCTYPE html>
<html lang="en">
<head>
	<meta name="viewport" content="width=device-width, initial-scale=1">
	<meta http-equiv="content-type" content="text/html; charset=utf-8" />
	<title>Design School Changlog</title>
	<meta name="Description" content="List of daily changes to the site">
	<link rel="stylesheet" type="text/css" charset="utf-8"  media="screen, projection" href="/ds.css?<?php echo $cachebuster; ?>" />
	<link rel="apple-touch-icon" sizes="180x180" href="/apple-touch-icon.png">
	<link rel="icon" type="image/png" sizes="32x32" href="/favicon-32x32.png">
	<link rel="icon" type="image/png" sizes="16x16" href="/favicon-16x16.png">
	<link rel="manifest" href="/site.webmanifest">
</head>
<body>
	<?php include $_SERVER['DOCUMENT_ROOT'] . '/_header.php'; ?>
	<main>
		<?php include $_SERVER['DOCUMENT_ROOT'] . '/_nav.php'; ?>
		<article>
			<hgroup>
				<h1>Changelog</h1>
				<h2>An ongoing list of updates to this site</h2>
				<small>Design inspired by <a href="https://carrd.co/docs/general/changelog" rel="nofollow">Carrd</a></small>
			</hgroup>
			<div class="changelog">
				<div class="day">
					<h3>Apr 18, 2023</h3>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Leadership: Give credit to your team</p>
						  </div>
					</div>
				</div>
				<div class="day">
					<h3>Mar 29, 2023</h3>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Leadership: Listen Intently</p>
						  </div>
					</div>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Leadership: Ask Questions</p>
						  </div>
					</div>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Leadership: Own Your Mistakes</p>
						  </div>
					</div>
				</div>
				<div class="day">
					<h3>Mar 23, 2023</h3>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Leadership: Trust & Respect</p>
						  </div>
					</div>
				</div>
				<div class="day">
					<h3>Mar 22, 2023</h3>
					<div class="task">
						  <span class="badge updated">updated</span>
						  <div class="desc">
								<p>Leadership: Everyone's playbook is different</p>
						  </div>
					</div>
					<div class="task">
						  <span class="badge updated">updated</span>
						  <div class="desc">
								<p>Leadership: Priorities</p>
						  </div>
					</div>
				</div>
				<div class="day">
					<h3>Mar 21, 2023</h3>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Leadership: Actually care</p>
						  </div>
					</div>
				</div>
				<div class="day">
					<h3>Feb 25, 2021</h3>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Delegation lesson draft</p>
						  </div>
					</div>
				</div>
				<div class="day">
					<h3>Feb 24, 2021</h3>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Start every day with a plan lesson draft</p>
						  </div>
					</div>
				</div>
				<div class="day">
					<h3>Feb 22, 2021</h3>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Letting go of design work lesson draft</p>
						  </div>
					</div>
				</div>
				<div class="day">
					<h3>Feb 18, 2021</h3>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Time management lesson draft</p>
						  </div>
					</div>
				</div>
				<div class="day">
					<h3>Feb 17, 2021</h3>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Things to consider lesson draft</p>
						  </div>
					</div>
				</div>
				<div class="day">
					<h3>Feb 12, 2021</h3>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Priorities lesson draft</p>
						  </div>
					</div>
				</div>
				<div class="day">
					<h3>Feb 4, 2021</h3>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Primary objective lesson draft</p>
						  </div>
					</div>
				</div>
				<div class="day">
					<h3>Feb 2, 2021</h3>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Stubbed out course pages</p>
						  </div>
					</div>
				</div>
				<div class="day">
					<h3>Feb 1, 2021</h3>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Draft status</p>
						  </div>
					</div>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Lesson version of data set</p>
						  </div>
					</div>
				</div>
				<div class="day">
					<h3>Jan 26, 2021</h3>
					<div class="task">
						  <span class="badge updated">updated</span>
						  <div class="desc">
								<p>Status labels to be more descriptive</p>
						  </div>
					</div>
					<div class="task">
						  <span class="badge updated">updated</span>
						  <div class="desc">
								<p>Dynamic cache buster</p>
						  </div>
					</div>
				</div>
				<div class="day">
					<h3>Jan 18, 2021</h3>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Updates signup form</p>
						  </div>
					</div>
				</div>
				<div class="day">
					<h3>Jan 13, 2021</h3>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Outline for course pages</p>
						  </div>
					</div>
				</div>
				<div class="day">
					<h3>Jan 9, 2021</h3>
					<div class="task">
						  <span class="badge updated">updated</span>
						  <div class="desc">
								<p>Next/Prev links</p>
						  </div>
					</div>
				</div>
				<div class="day">
					<h3>Jan 8, 2021</h3>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Next/Prev links</p>
						  </div>
					</div>
				</div>
				<div class="day">
					<h3>Jan 6, 2021</h3>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Favicon</p>
						  </div>
					</div>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Updates for lighthouse score</p>
						  </div>
					</div>
				</div>
				<div class="day">
					<h3>Jan 1, 2021</h3>
					<div class="task">
						  <span class="badge updated">updated</span>
						  <div class="desc">
								<p>Data structure for courses and lessons</p>
						  </div>
					</div>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Remaining leadership course lessons</p>
						  </div>
					</div>
				</div>
				<div class="day">
					<h3>Dec 31, 2020</h3>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p><a href="/leadership/#fundamentals">Leadership fundamentals</a> section</p>
						  </div>
					</div>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p><a href="/leadership/#communication">Leadership communication</a> section</p>
						  </div>
					</div>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Hash links to headers of course pages</p>
						  </div>
					</div>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Progress pill-box to lessons</p>
						  </div>
					</div>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Sharing links</p>
						  </div>
					</div>
					<div class="task">
						<span class="badge updated">updated</span>
						<div class="desc">
							<p>Mobile styles</p>
						</div>
					</div>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p><a href="/leadership/#organization">Leadership organization</a> section</p>
						  </div>
					</div>
				</div>
				<div class="day">
					<h3>Dec 30, 2020</h3>
					<div class="task">
						  <span class="badge updated">updated</span>
						  <div class="desc">
								<p>JS to PHP</p>
						  </div>
					</div>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Breadcrumbs navigation</p>
						  </div>
					</div>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Cache busting</p>
						  </div>
					</div>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Contributor section</p>
						  </div>
					</div>
				</div>
				<div class="day">
					<h3>Dec 29, 2020</h3>
					<div class="task">
						  <span class="badge added">added</span>
						  <div class="desc">
								<p>Initial site structure</p>
						  </div>
					</div>
				</div>
				<div class="day">
					<h3>Dec 28, 2020</h3>
					<div class="task">
						<span class="badge added">added</span>
						<div class="desc">
							<p>Edit modal</p>
						</div>
					</div>
				</div>
				<div class="day">
					<h3>Dec 23, 2020</h3>
					<div class="task">
						<span class="badge added">added</span>
						<div class="desc">
							<p><a href="/changelog/">Changelog</a> page</p>
						</div>
					</div>
					<div class="task">
						<span class="badge added">added</span>
						<div class="desc">
							<p>Responsive styles</p>
						</div>
					</div>
					<div class="task">
						<span class="badge updated">updated</span>
						<div class="desc">
							<p>Copy across all pages</p>
						</div>
					</div>
				</div>
				<div class="day">
					<h3>Dec 22, 2020</h3>
					<div class="task">
						<span class="badge added">added</span>
						<div class="desc">
							<p><a href="/about/">About</a> page</p>
						</div>
					</div>
					<div class="task">
						<span class="badge added">added</span>
						<div class="desc">
							<p><a href="/">Home</a> page</p>
						</div>
					</div>
				</div>
			</div>
		</article>
	</main>
	<script type="text/javascript" src="/ds.js?<?php echo $cachebuster; ?>"></script>
</body>
</html>